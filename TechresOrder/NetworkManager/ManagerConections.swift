...
enum ManagerConections: TargetType {
    ...
    case orders(userId:Int, order_status:String, key_word:String = "", branch_id:Int = -1)
    case order(order_id:Int, branch_id:Int)
    case foods(branch_id:Int, area_id:Int = -1, category_id:Int, category_type:Int, is_allow_employee_gift:Int = -1, is_sell_by_weight:Int = -1, is_out_stock:Int = 0, key_word:String = "")
    ...
    case discount(order_id:Int, branch_id:Int,discount_type:Int, discount_percent:Int, note:String)
    ...
    case createArea(branch_id:Int, areaRequest:AreaRequest)
    case foodsManagement(branch_id:Int, status:Int = -1, category_types:Int = -1, restaurant_kitchen_place_id:Int = -1)
    case categoriesManagement(branch_id:Int, status:Int = -1)
    ...
    case ordersHistory(id:Int, report_type:Int, time : String, limit : Int, page : Int, key_search:String)
    case units
    case createFood(branch_id:Int, foodRequest:CreateFoodRequest)
    case generateFileNameResource(medias:[Media])
    case updateFood(branch_id:Int, foodRequest:CreateFoodRequest)
...
//    case vats
    ...
    //API REPORT SEEMT
        ...
        case getRestaurantPLReport(restaurant_brand_id:Int, branch_id: Int, report_type:Int, date_string:String, from_date:String, to_date:String) //@*13
        ...
        case healthCheckChangeDataFromServer(branch_id:Int, restaurant_brand_id: Int)
}
extension ManagerConections {
    ...
    var path: String {
        ...
        case .ordersHistory(_, _, _, _, _, _):
            let parameter = parameters! as NSDictionary
...
//            let order_id = parameter.object(forKey: "order_id") as? Int
        ...
        case .getReportOrderRestaurantDiscountFromOrder(_, _, _, _, _, _):
              ...
              case .getRestaurantPLReport(_, _, _, _, _, _):
                  return APIEndPoint.NameReportEndPoint.urlRestaurantPLReport
        ...
        case .healthCheckChangeDataFromServer(branch_id: let branch_id, restaurant_brand_id: let restaurant_brand_id):
    ...
    var headers: [String : String]? {
        ...
        case .getReportOrderRestaurantDiscountFromOrder(_, _, _, _, _, _):
                  return headerNode(ProjectId: Constants.PROJECT_IDS.PROJECT_ID_REPORT, Method: Constants.METHOD_TYPE.GET)//@
              case .getOrderReportFoodCancel(_, _, _, _, _, _, _):
                  return headerNode(ProjectId: Constants.PROJECT_IDS.PROJECT_ID_REPORT)//@
              case .getOrderReportFoodGift(_, _, _, _, _, _, _, _):
                  return headerNode(ProjectId: Constants.PROJECT_IDS.PROJECT_ID_REPORT)
              case .getRestaurantRevenueCostProfitEstimation(_,_,_,_,_,_):
                  return headerNode(ProjectId: Constants.PROJECT_IDS.PROJECT_ID_REPORT)
    ...
    var parameters: [String: Any]? {
        ...
        case .orders(let userId, let order_status, let key_word, let branch_id):
            return ["employee_id": userId, "order_status": order_status, "key_word": key_word, "branch_id":branch_id, "is_take_away":DEACTIVE]
            
        case .order(let order_id, let branch_id):
            return ["id": order_id, "branch_id": branch_id]
            
        case .foods(let branch_id, let area_id, let category_id, let category_type, let is_allow_employee_gift, let is_sell_by_weight, let is_out_stock, let keyword):
            
            return ["branch_id": branch_id, "area_id": area_id, "category_id": category_id, "category_type": category_type, "is_allow_employee_gift": is_allow_employee_gift, "is_sell_by_weight": is_sell_by_weight, "is_out_stock": is_out_stock, "keysearch": keyword, "is_get_restaurant_kitchen_place": ALL, "status":ACTIVE]
        ...
        case .discount(let order_id, let branch_id,  let discount_type, let discount_percent, let note):
            
            return ["order_id":order_id , "branch_id": branch_id, "discount_type": discount_type, "discount_percent": discount_percent, "note": note]
        ...
        case .extra_charges(let restaurant_brand_id, let branch_id,  let status):
            ...
            return ["restaurant_brand_id":restaurant_brand_id, "branch_id": branch_id, "status": status]
        ...
        case .createArea(let branch_id, let areaRequest):
            
            return ["branch_id":branch_id,"name": areaRequest.name, "status":areaRequest.status, "id": areaRequest.id]
            
        case .foodsManagement(let branch_id, let status, let category_types, let restaurant_kitchen_place_id):
            
            return ["branch_id":branch_id,"status": status,"category_types": category_types,"restaurant_kitchen_place_id": restaurant_kitchen_place_id]
            
        case .categoriesManagement(let branch_id, let status):
        ...
        case .ordersHistory(let id, let report_type,let time, let limit, let page, let key_search):
            return [
                "id":id,
                "report_type":report_type,
                "time": time,
...
//            return ["from_order_id": order_id, "destination_table_id":destination_table_id, "to_table_id": target_table_id,  "branch_id": branch_id, "list_extra_charge": foods.toJSON()]
        ...
        case .getReportOrderRestaurantDiscountFromOrder(let restaurant_brand_id,  let branch_id, let report_type, let date_string, let from_date, let to_date):
                ...
                case .getRestaurantPLReport(let restaurant_brand_id, let branch_id, let report_type, let date_string, let from_date, let to_date):
                    return [
                        "restaurant_brand_id":restaurant_brand_id,
                        "branch_id":branch_id,
                        "report_type":report_type,
...
//    }
    ...
    let appServiceProvider = MoyaProvider<ManagerConections>(endpointClosure: endpointClosure, plugins: [networkLogger, NetworkActivityPlugin(networkActivityClosure: { (activity, target) in
                switch activity
                {
                case .began:
