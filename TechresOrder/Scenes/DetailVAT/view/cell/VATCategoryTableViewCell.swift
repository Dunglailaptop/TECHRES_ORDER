...
class VATCategoryTableViewCell: UITableViewCell {
    ...
    public var data: VATOrder? = nil {
        didSet {
            lbl_vat_total.text = Utils.stringVietnameseMoneyFormatWithNumberDouble(amount: data!.vat_amount)
...
extension VATCategoryTableViewCell:UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let order_detail = self.order_details[indexPath.row] as! DetailVAT
        cell.food_name.text = order_detail.name
        //nhân số tiền với số lượng thêm khi đặt món
        cell.food_price.text = Utils.stringVietnameseMoneyFormatWithNumberDouble(amount: (order_detail.price * order_detail.quantity))
        cell.food_vat_price.text = Utils.stringVietnameseMoneyFormatWithNumberDouble(amount: order_detail.vat_amount)
        cell.lbl_discount.text = String(format: "Giảm giá (%d%%): %@",Int(order_detail.discount_percent),Utils.stringVietnameseMoneyFormatWithNumberDouble(amount: order_detail.discount_amount))
