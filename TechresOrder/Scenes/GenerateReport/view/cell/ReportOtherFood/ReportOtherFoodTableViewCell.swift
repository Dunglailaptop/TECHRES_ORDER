//
//  ReportOtherFoodTableViewCell.swift
//  Techres-Seemt
//
...
import RxSwift
import ObjectMapper
class ReportOtherFoodTableViewCell: UITableViewCell {
    ...
    @IBAction func actionChooseReportType(_ sender: UIButton) {
        ...
        var otherFoodReport = viewModel.otherFoodReport.value
        otherFoodReport.foods = []
        otherFoodReport.reportType = sender.tag
        otherFoodReport.dateString = viewModel.view?.reportTypeArray[sender.tag] ?? ""
    ...
    @IBAction func actionDetail(_ sender: Any) {
        guard let viewModel = self.viewModel else {return}
        viewModel.makeToDetailReportOtherFoodViewController()
    }
}
