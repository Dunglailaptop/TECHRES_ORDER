...
//            Toast.show(message: "Số lượng không phù hợp", controller: self)
...
//                Toast.show(message: "Số lượng tối đa cho phép là 1000", controller: self)
                JonAlert.showError(message: String(format: "Số lượng tối đa là %.2f", max_quantity!), duration: 2.0)
            } else {
                delegate_quantity?.callbackCaculatorInputQuantity(number: Float(self.textField_Result.text ?? "0")!, position: position)
                        dismiss(animated: true, completion: nil)
            }
        } else {
            if (self.textField_Result.text! as NSString).floatValue > max_quantity! {
//                Toast.show(message: "Số lượng tối đa cho phép là 200", controller: self)
        ...
        /*
        if isCheckQuantity == 1 {
            if is_sell_by_weight == 0 {
                if (self.textField_Result.text! as NSString).floatValue > 1000 {
                    Toast.show(message: "Số lượng tối đa cho phép là 1000", controller: self)
                } else {
                    delegate_quantity?.callbackCaculatorInputQuantity(number: Float(self.textField_Result.text ?? "0")!, position: position)
                            dismiss(animated: true, completion: nil)
                ...
                if (self.textField_Result.text! as NSString).floatValue > 200 {
                    Toast.show(message: "Số lượng tối đa cho phép là 200", controller: self)
                } else {
                    delegate_quantity?.callbackCaculatorInputQuantity(number: Float(self.textField_Result.text ?? "0")!, position: position)
                            dismiss(animated: true, completion: nil)
...
//            if (self.textField_Result.text! as NSString).floatValue > self.current_quantity! {
//                Toast.show(message: "Số lượng không phù hợp", controller: self)
//            } else {
//                delegate_quantity?.callbackCaculatorInputQuantity(number: Float(self.textField_Result.text ?? "0")!, position: position)
//                //        delegate_quantity?.callBackQuantity(quantity: Int(self.textField_Result.text!)!,position:position!)
//                        dismiss(animated: true, completion: nil)
//            }
