...
class ReportBusinessViewController: BaseViewController {
    ...
    var categoryReport:RevenueCategoryReport = RevenueCategoryReport.init(reportType: REPORT_TYPE_TODAY, dateString: Utils.getCurrentDateTime().dateTimeNow)
    var areaRevenueReport:AreaRevenueReport = AreaRevenueReport.init(reportType: REPORT_TYPE_TODAY, dateString: Utils.getCurrentDateTime().dateTimeNow)
    var tableRevenueReport:TableRevenueReport = TableRevenueReport.init(reportType: REPORT_TYPE_TODAY, dateString: Utils.getCurrentDateTime().dateTimeNow)
    var foodReport:FoodReportData = FoodReportData.init(reportType: REPORT_TYPE_TODAY, dateString: Utils.getCurrentDateTime().dateTimeNow)
    var cancelFoodReport:FoodReportData = FoodReportData.init(reportType: REPORT_TYPE_TODAY, dateString: Utils.getCurrentDateTime().dateTimeNow)
    var giftReport:FoodReportData = FoodReportData.init(reportType: REPORT_TYPE_TODAY, dateString: Utils.getCurrentDateTime().dateTimeNow)
    var discountReport:DiscountReport = DiscountReport.init(reportType: REPORT_TYPE_TODAY, dateString: Utils.getCurrentDateTime().dateTimeNow)
    var vatReport:VATReport = VATReport.init(reportType: REPORT_TYPE_TODAY, dateString: Utils.getCurrentDateTime().dateTimeNow)
...
//    var report_type = REPORT_TYPE_TODAY
    ...
    var cates = [Category]()
    var cate_names = ["DANH MỤC", "MÓN ĂN", "MÓN HỦY", "MÓN TẶNG", "GIẢM GIÁ", "VAT","BÀN","KHU VỰC"]
...
//        getCurentTime()
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        avatar_branch.kf.setImage(with: URL(string: Utils.getFullMediaLink(string: ManageCacheObject.getCurrentBranch().avatar)),placeholder: UIImage(named: "image_defauft_medium")) // hình ảnh
...
extension ReportBusinessViewController: BmoViewPagerDataSource, BmoViewPagerDelegate{
    ...
    func bmoViewPagerDataSource(_ viewPager: BmoViewPager, viewControllerForPageAt page: Int) -> UIViewController {
        switch page{
            ...
            case 2://MARK: Báo cáo món huỷ
                let vc = ReportBusinessCancelFoodViewController()
                vc.viewModel = self.viewModel
                return vc
                
            
            case 3://MARK: Báo cáo món tăng
            ...
            case 4://MARK: Báo cáo giảm giá
                let vc = ReportBusinessDiscountViewController()
                vc.viewModel = self.viewModel
                return vc
            
            case 5://MARK: Báo cáo doanh thu VAT
                let vc = ReportBusinessVATViewController()
                vc.viewModel = self.viewModel
                return vc
            
            
            case 6://MARK: Báo cáo doanh thu bàn
                let vc = ReportBusinessTableRevenueViewController()
                vc.viewModel = self.viewModel
                return vc
            
            case 7://MARK: Báo cáo doanh thy khu vực
...
//        return CGSize(width: (navigationBar.bounds.width)/7, height: navigationBar.bounds.height)
    }
