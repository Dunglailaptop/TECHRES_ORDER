...
//        if(current_quantity! > 0 && max_quantity! > current_quantity!){
//            JonAlert.showSuccess(message: String(format: "Bạn không thể nhập quá số lượng %d", current_quantity!), duration: 2.0)
//        }
        delegate?.callbackCaculatorInputQuantity(number: Float(self.textField_Result.text!)!, position: position)
        dismiss(animated: true, completion: nil)
    }
