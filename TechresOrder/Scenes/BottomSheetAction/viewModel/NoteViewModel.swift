...
extension NoteViewModel{
    func notes() -> Observable<APIResponse> {
        return appServiceProvider.rx.request(.notes(branch_id: branch_id.value))
    ...
    func notesByFood() -> Observable<APIResponse> {
        return appServiceProvider.rx.request(.notesByFood(food_id: order_detail_id.value, brand_id: branch_id.value))
               .filterSuccessfulStatusCodes()
               .mapJSON().asObservable()
               .showAPIErrorToast()
               .mapObject(type: APIResponse.self)
